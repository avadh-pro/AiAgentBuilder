 Discovery is done. Key findings:
  ...
  - Code is single-tenant deep{So we need this flexible enough right }: @amarolabs hardcoded, source allowlist baked into scorer.py{for different niche user may want to have the different source of news}, no tenant_id anywhere, skills loaded from per-machine ~/.claude/skills/. The spec §1.3
  explicitly lists multi-tenant + Instagram posting + web dashboard as v1 non-goals
    ↳ **YES.** Today the pipeline fits ONE shop (@amarolabs) like a tailored suit. To sell it, the suit must become adjustable — brand name, sources, voice, visual style all changeable WITHOUT touching code.
    ↳ **Exactly right.** A doctor trusts WHO/Lancet. A finance creator trusts Bloomberg. A lawyer trusts SCOTUSblog. Today this list is hardcoded in Python (`scorer.py:25-54`) — adding a new customer means an engineer edits code + redeploys. SaaS-ready = each customer keeps their OWN list, in a row they can edit themselves.

  - Skills are the editorial moat (3 new: amarolabs-news-carousel, neuro-carousel-writer, amara-news-carousel). {What do you mean by skill being editorial moat}
    ↳ **"Moat" = the water ditch around a castle that keeps invaders out.** In business it means anything competitors CAN'T easily copy. Your skill files (markdown) encode YOUR taste — voice, sources, visual style. Anyone can copy our Python code in a weekend. Nobody can copy YOUR taste. The defense lives in the skill files, not the code. That's why "skills = editorial moat".

  - Existing strategy in Brainstorm/ already points at AgentShield {What is AgentShield in context of Carousel ?} + vertical SaaS as the moat-y{what is moat-y ?} plays, with "build the media company first" {yes I would but things is I need to have the product read side by side} as the distribution thesis.
    ↳ **AgentShield is NOT the carousel product.** It's a DIFFERENT business idea from your earlier `Brainstorm/1stSession.md` — a tool that helps companies safely run AI agents in production (guardrails, monitoring). I mentioned it only to remind you: carousel SaaS isn't your only option. Keep other paths visible before locking in.
    ↳ **"moat-y" = informal for "has a strong moat" = hard to copy.** Vertical SaaS (one product for ONE industry, e.g. only dental clinics) is moat-y because niche knowledge + integrations + customer relationships pile up over years, and giants like OpenAI won't chase such small slices. Generalist tools have no moat; specialists do.
  ...
  carousels for regulated/expert niches is an open seam. { you are saying market is distributed what do you mean by this ? Does in work in favour of me }
    ↳ **"Bifurcated" = "split into two" (bi = two).** Today's carousel market has two camps:
        • **Volume camp** ("post 30/day, fast & cheap") → Predis $19, Postnitro — sells to solo creators
        • **Approval camp** ("show client → sign-off → post") → Planable, ContentStudio — sells to agencies
      Nobody is selling **"credible, source-cited carousels for serious experts who can't be wrong"** (doctors, biotech, lawyers, finance). That empty chair at the table = YOUR opening. **YES, it works strongly in your favor** — @amarolabs already proves this style works; nobody else is productizing it.

  - Hard constraints: Instagram Graph API needs 4–6 week app review + Business account{How does this harm my carousel application }; gpt-image-2 medium ≈ $0.053/image (10 slides × 30/mo ≈ $15.90 just in images); per-seat
  pricing is "business-model debt"{what is business-model debt } in 2026; carousel SaaS dies via Canva Magic Studio shipping equivalents free + one-and-done usage
    ↳ **To AUTO-POST to Instagram on a customer's behalf, Meta makes you apply + wait 4–6 weeks for review.** Plus the customer's IG must be a Business account. So if you promise "we post for you", you're stuck waiting weeks before you can deliver. **Smart workaround for v1:** skip auto-posting entirely. Hand the customer a ZIP of slide PNGs + a caption.txt file — they paste it into IG themselves in 30 seconds. Same end result, zero Meta waiting, zero suspension risk.
    ↳ **"Business-model debt" = like technical debt, but for pricing.** Tech debt = bad code today, expensive to fix later. Business-model debt = a pricing model that works today but bites you later. Per-seat pricing ($X per user/month) is dying for AI tools because ONE heavy user can burn $200 of OpenAI credits in a day → you LOSE money on them. If you start per-seat now, in a year you'll have to migrate every customer onto "base fee + usage credits" and they'll be angry. **Better: pick the right model from day one** (e.g., "$199/mo gets you 30 approved carousels; $5 each after that").
